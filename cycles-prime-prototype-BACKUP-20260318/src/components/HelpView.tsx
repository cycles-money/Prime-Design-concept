...
function Section({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="bg-white dark:bg-[var(--color-1)] rounded-2xl shadow-md overflow-hidden">
      <div className="flex items-center gap-2.5 px-5 py-3.5 border-b border-gray-200 dark:border-[var(--border)] bg-gray-50 dark:bg-[var(--surface-3)]">
      ...
      <div className="max-w-3xl mx-auto w-full px-6 py-6 space-y-5">
        ...
        <Section title="Batch Statuses" icon={IconBatch}>
          ...
          <dl>
            ...
            />
            <StatusRow
              badge={<Badge label="Included in Cycle" color="positive" />}
              title="Included in Cycle"
              description="The batch has been picked up by a netting cycle and will be settled in that run. After the cycle executes, cleared and remaining amounts are updated."
            />
          </dl>

          {/* Flow arrow */}
          <div className="mt-4 flex items-center gap-1.5 flex-wrap">
            {(['Draft', 'Pending', 'Ascertained', 'Included in Cycle'] as const).map((s, i, arr) => (
        ...
        <Section title="End-to-End Workflow" icon={IconFlow}>
          ...
          <div>
            ...
            <FlowStep
              ...
              sub="At the scheduled cut-off time, all Ascertained batches are picked up. Multilateral netting is applied across all counterparties. Status: Included in Cycle."
            />
            <FlowStep
              step={4}
