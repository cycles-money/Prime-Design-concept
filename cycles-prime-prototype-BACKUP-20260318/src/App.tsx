...
import { ClipboardList, RefreshCw, Users, Settings, CircleHelp, Sun, Moon } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <DarkModeContext.Provider value={isDark}>
      ...
      <div className="flex flex-col h-screen font-sans overflow-hidden bg-gray-50 dark:bg-[var(--surface-2)] transition-colors duration-150">
        {/* ── Top navigation bar ──────────────────────────────────────────── */}
        <header className="bg-white dark:bg-[var(--color-1)] border-b border-gray-200 dark:border-[var(--border)] relative flex items-center flex-shrink-0 h-12 transition-colors duration-150 px-3">
        ...
        <main id="main-content" className="flex-1 overflow-hidden">
          {activeTab === 'batches' ? <BatchesView batches={batches} onBatchesChange={setBatches} /> : activeTab === 'cycles' ? <CyclesView batches={batches} /> : activeTab === 'counterparties' ? <CounterpartiesView /> : activeTab === 'settings' ? <SettingsView /> : <HelpView />}
        </main>
      </div>
